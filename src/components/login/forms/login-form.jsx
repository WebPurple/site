...
const renderInput = ({ meta, input, ...props }) => (
  <Input {...props} {...input} />
)
...
const LoginForm = ({ onShowRegisterForm, handleSubmit, onSubmit }) => (
  <div>
    <SocialButton
      bgColor={fbColor.string()}
      bgColorHover={fbColorHover}
      href="/auth/fb">
      <FacebookIcon />
      <SocialButtonText>Login with Facebook</SocialButtonText>
    </SocialButton>
    <SocialButton
      bgColor={vkColor.string()}
      bgColorHover={vkColorHover}
      href="/auth/vk">
      <VkWipIcon />
    ...
    <form onSubmit={handleSubmit(event => onSubmit(event))}>
      ...
      />
      <FormFooter>
        <PopupLinkButton onClick={preventDefault(onShowRegisterForm)}>
          Create account
        </PopupLinkButton>
